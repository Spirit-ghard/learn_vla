...
## 2026-09-03：将 FlashRT 纳入真实落地加速路线

背景：

- 仅完成数据采集、训练和“能推理”不足以支撑真实机器人部署。
- 真实系统还必须处理推理延迟、相机传输、显存、功耗、资源争用、动作安全、故障恢复和长期运行。

决策：

- 将 FlashRT 记录为 VLA 推理运行时加速候选，而不是完整的机器人资源管理方案。
- 先对官方支持的模型和硬件建立原始 PyTorch/LeRobot 基线，再接入 FlashRT 做 A/B 测试。
- 当前项目的 ACT checkpoint 不默认承诺可直接由 FlashRT 加速；若不兼容，先保留原生 ACT 推理。
- 真实机器人最终采用本机闭环：低级控制、安全 watchdog、急停和动作限幅不能依赖公网或远程策略服务。
- 远程异步 chunk 方案继续用于仿真和研发验证；真实部署需额外通过断网、丢帧、超时、GPU 溢出和服务重启测试。

必须记录的部署指标：

- 相机曝光到动作执行的端到端 P50/P95/P99 延迟。
- 策略推理 P50/P95/P99、冷启动时间和动作输出频率。
- GPU 显存峰值/余量、GPU 利用率、CPU 占用、功耗和温度。
- 相机有效帧率、丢帧率、时间戳偏差、网络抖动和动作队列欠载次数。
- 动作限幅次数、watchdog 触发次数、故障恢复时间和降级控制是否成功。
