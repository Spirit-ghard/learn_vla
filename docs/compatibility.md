...
源码参考，项目运行时不导入 `leisaac` Python 包。SO101 USD 资产已复制进本项目目录。
...
- 默认使用当前已激活 Python；如果当前 Python 低于 3.10，会尝试常见的 `lwh_isaac` conda 环境；需要跨环境重执行时通过 `LWH_ISAAC_PYTHON` 指定。
- 配置 Isaac Sim 的应用、扩展、Python 和动态库路径。
- 校验项目内置 SO101 USD 已完整下载。
...
仍然保留的 LeIsaac 关联只有源码参考和历史对照；项目默认资产来自
...
仿真关节顺序与 LeIsaac SO101 配置和本机历史 ROS 工程检查结果一致：
...
仿真使用项目内置 `so101_follower.usd` 作为模型和关节限位真值。
